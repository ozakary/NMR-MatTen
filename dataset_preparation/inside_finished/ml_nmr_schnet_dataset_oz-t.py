...
def extract_shielding_tensors(mpshift_path, atomic_coordinates, structure_id):
    ...
    tensor_data = []
    molecule_name = f'structure_{structure_id}'
    ...
    try:
        ...
        for atom_line in atom_lines:
            ...
            if len(parts) == 4:  # Ensure that there are exactly 4 columns
                atom, x, y, z = parts
                coordinates.append([f'structure_{structure_id}', len(coordinates), atom, float(x), float(y), float(z)])
    ...
    for idx, folder in enumerate(cluster_folders):
        ...
        if os.path.exists(xyz_path):
            ...
            if os.path.exists(mpshift_path):
                ...
                output_path_tensors = os.path.join(output_dir_tensors, f'structure_{structure_id}.csv')
                output_path_coordinates = os.path.join(output_dir_coordinates, f'structure_{structure_id}.csv')
                ...
                for i in range(len(atomic_coordinates)):
                    train_data.append([len(train_data), f'structure_{structure_id}'])
            else:
